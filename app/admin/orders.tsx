...
import { getAdminOrders, markAdminOrderPaid, performAdminOrderAction, type AdminOrder, type StaffOrderAction } from "@/lib/admin-api";
...
export default function AdminOrdersScreen() {
  ...
  const actionCopy = pendingAction ? {
    title: pendingAction.action === "make_available" ? "Release order to drivers?" : pendingAction.action === "ready_for_delivery" ? "Start the delivery queue?" : pendingAction.action === "unassign_driver" ? "Remove the assigned driver?" : "Confirm cash payment?",
    message: pendingAction.action === "make_available" ? "Drivers will immediately see and be able to accept this order. Your name will be stamped on the release." : pendingAction.action === "ready_for_delivery" ? "This creates a fresh delivery task and records who released it." : pendingAction.action === "unassign_driver" ? "The task will return to the open driver queue. This is restricted to superadmins." : `Record ${formatNaira(pendingAction.order.paidAmount)} as collected? This payment record cannot be changed.`,
    label: pendingAction.action === "make_available" ? "Make available" : pendingAction.action === "ready_for_delivery" ? "Release delivery" : pendingAction.action === "unassign_driver" ? "Unassign driver" : "Mark paid",
  } : null;
...
function OrderCard({ order, role, working, onAction }: { order: AdminOrder; role: AppRole; working: boolean; onAction: (order: AdminOrder, action: OrderUiAction) => void }) {
  ...
  return <View style={styles.card}>
    ...
    <View style={[styles.releaseStamp, order.availableToDrivers ? styles.releaseStampActive : styles.releaseStampHeld]}><Ionicons name={order.availableToDrivers ? "shield-checkmark" : "pause-circle-outline"} size={14} color={order.availableToDrivers ? "#0C8A63" : "#8A5B00"} /><View style={{ flex: 1 }}><Text style={styles.releaseTitle}>{order.availableToDrivers ? `Released by ${order.availableByName || "Operations"}` : "Held from driver queue"}</Text><Text style={styles.releaseDetail}>{order.availableToDrivers ? `${order.availableByRole === "system" ? "Automatic after payment" : "Staff-approved dispatch"}${order.availableAt ? ` · ${formatDayOnly(order.availableAt)}` : ""}` : "Use Make available when the order is ready for a driver."}</Text></View></View>
